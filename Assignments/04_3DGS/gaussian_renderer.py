...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        J_proj[:, 0, 2] = -cam_points[:, 0] * K[0, 0] / depths**2
        J_proj[:, 1, 1] = K[1, 1] / depths
        J_proj[:, 1, 2] = -cam_points[:, 1] * K[1, 1] / depths**2

        # Transform covariance to camera space
        # FILL: Aplly world to camera rotation to the 3d covariance matrix
        # covs_cam = ...  # (N, 3, 3)
        covs_cam = torch.matmul(R.unsqueeze(0), torch.matmul(
            covs3d, R.unsqueeze(0).permute(0, 2, 1)))
        # Project to 2D
        covs2D = torch.bmm(J_proj, torch.bmm(
            covs_cam, J_proj.permute(0, 2, 1)))  # (N, 2, 2)
    ...
    def compute_gaussian_values(
        ...
        # gaussian = ... ## (N, H, W)
        power = -0.5 * torch.einsum('nhwi,nij,nhwj->nhw',
                                    dx, torch.inverse(covs2D), dx)
        gaussian = torch.exp(power) / (2 * np.pi *
                                       torch.sqrt(torch.det(covs2D).view(N, 1, 1)))
        return gaussian
    ...
    def forward(
        ...
        gaussian_values = self.compute_gaussian_values(
            means2D, covs2D, self.pixels)  # (N, H, W)

        # 5. Apply valid mask
        gaussian_values = gaussian_values * \
            valid_mask.view(N, 1, 1)  # (N, H, W)

        # 6. Alpha composition setup
        alphas = opacities.view(N, 1, 1) * gaussian_values  # (N, H, W)
        colors = colors.view(N, 3, 1, 1).expand(-1, -1,
                                                self.H, self.W)  # (N, 3, H, W)
        colors = colors.permute(0, 2, 3, 1)  # (N, H, W, 3)

        # 7. Compute weights
        # FILL:
        # weights = ... # (N, H, W)
        weights = alphas * (1-alphas).cumprod(dim=0)
        # 8. Final rendering
